...
test "slice_decoders.invariants: Mach-O decode populates sections, segments, imports, and exports" {
    ...
    for (desc.imports) |imp| {
        ...
        if (std.mem.indexOf(u8, imp, "printf")) | _idx | { found_printf_or_malloc = true; };
        if (std.mem.indexOf(u8, imp, "malloc")) | _idx | { found_printf_or_malloc = true; };
        if (std.mem.indexOf(u8, imp, "dyld_stub_binder")) | _idx | { found_dyld_stub = true; };
    }
    ...
    if (desc.exports.len > 0) {
        ...
        for (desc.exports) |ex| {
            ...
            if (std.mem.indexOf(u8, ex.name, "__mh_execute_header")) | _idx | { found_mh_header = true; };
        }
